...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    const records = await Consumable.find(query).sort({ date: -1 });
    res.json(records);
  } catch (err) {
...
router.post('/', authMiddleware, async (req, res) => {
  const record = new Consumable(req.body);
  try {
...
router.put('/:id', authMiddleware, authorizeRoles('Admin', 'Manager'), async (req, res) => {
  try {
    const updatedRecord = await Consumable.findByIdAndUpdate(req.params.id, req.body, { new: true });
